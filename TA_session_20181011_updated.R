...
#Probability Space
samplespace <- c(1,-1) #outcome
p <- c(1/2, 1/2)

#sample without replacement
sample(x = samplespace, 
...
#' ### Examples of sum, abs, colSums

# Summing Up all values
sum(trials)
sum(trials)/n
mean(trials) # Similar to mean

# Absolute values
abs(trials)
min(abs(trials)) # is larger than 0

# Column Sums
trials10 <- matrix(trials,nrow=100,ncol=10) 
 # Just Make trials as a 100 coin toss for 10 times. 
colSums(trials10)
sum(trials10[,1]) == colSums(trials10)[1] # 1st element is 1st column sum
sum(trials10[,10]) == colSums(trials10)[10] # 10th element is 10th column sum

#+ include=FALSE 
## Render PDF Document 

#+ eval=FALSE, echo=FALSE
rmarkdown::render("TA_session_20181011_updated.R","pdf_document")
